  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0 relative z-10">
        ...
        <div className="hidden md:flex w-5/12 relative items-end overflow-hidden">
          <img
        ...
        {/* Right: services — full width on mobile */}
        <div className="w-full md:w-7/12 flex flex-col justify-center px-5 md:px-10 py-6 md:py-8 gap-4 md:gap-6">
          ...
          {/* Service cards — 2 column on all sizes */}
          <div className="grid grid-cols-2 gap-3 md:gap-4">
            {services.map((svc, i) => (
              <a
                ...
                className="flex flex-col items-center text-center gap-2 md:gap-3 p-4 md:p-5 rounded-xl transition-all hover:scale-[1.03] hover:shadow-lg shadow-sm"
                style={{ background: "rgba(127,127,127,0.7)", border: "1px solid rgba(127,127,127,0.3)" }}
              >
                <img src={svc.img} alt={svc.name} className="w-14 h-14 md:w-20 md:h-20 object-contain" />
                <div>
                  <p className="text-xs md:text-sm font-bold leading-tight text-white">{svc.name}</p>
                  <p className="text-xs mt-1 leading-snug text-white/75 hidden sm:block">{svc.desc}</p>
                </div>
              </a>
            ))}
          </div>
